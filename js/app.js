...
var input = document.querySelector("[name='dataEncry']");
...
function encriptarTexto() {
  ValidationMessage.classList.remove("alert-validacion");
  if (validarTextoExpReg.test(input.value) === true) {
    //? Cifrado de texto
    let resultEcriptacion = input.value.replace(
      /[aeiou]/g,
      (vocal) => llaveDeEncriptacion[vocal]
    );
    ...
    ValidationMessage.classList.add("alert-validacion");
  }
}

function descrifrarTexto() {
  let descencriptadorExpReg = new RegExp(
    Object.keys(llaveDeDesencriptacion).join("|"),
    "g"
  );
  let textoDescencriptado = input.value.replace(
    descencriptadorExpReg,
    (vocal) => llaveDeDesencriptacion[vocal]
  );
  elementoResult.innerHTML = `
...
async function copiarEncriptacion() {
  ...
  let boton = document.querySelector(".contenedor-boton-copiar .btn");
  try {
